            ...
            <div class="tm_round_border">
              <div class="tm_table_responsive">
                <table class="table">
                  <thead>
                    <tr>
                      <th class="tm_semi_bold tm_primary_color">Refferel's Name</th>
                      <th class="tm_semi_bold tm_primary_color">Test Name</th>
                      <th class="tm_semi_bold tm_primary_color tm_text_right">Reffered Amount</th>
                      <th class="tm_semi_bold tm_primary_color tm_text_right">Paid Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    @php
                      ...
                      $paid = 0;
                    @endphp
                    @foreach ($refferpay as $refferpay)
                    <tr>
                      <td class="">{{$refferpay->patinet->referral->name}}</td>
                      <td class="tm_text_right">
                        @foreach ($refferpay->patinet->tests as $test)
                        <span class="badge badge-primary">{{$test->name}}</span>
                        @endforeach  
                      </td>
                      <td class="">{{$refferpay->refd_amount}}</td>
                      <td class="">{{$refferpay->refferel_paid}}</td>
                    </tr>
                    @php
                      $total += $refferpay->refd_amount;
                      $paid += $refferpay->refferel_paid;
                    @endphp
                    @endforeach
                  </tbody>
