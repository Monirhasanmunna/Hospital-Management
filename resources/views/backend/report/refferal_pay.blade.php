...
<div class="container-fluid" id="container-wrapper">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800">Refferal Payment Report</h1>
      <ol class="breadcrumb">
            ...
            <div class="card">
                <div class="card-header m-auto">
                    <form class="form-inline">
                        ...
                        <a href="{{ route('app.report.refd') }}" class="btn btn-primary ml-5">Today</a> 
                    </form>
                </div>
            </div>
            <div class="card mb-4">
                ...
                <table class="table align-items-center table-flush table-hover" id="dataTableHover">
                  ...
                  <tbody>
                    @php
                        $total = 0;
                    @endphp
                    @foreach ($refferal_pay as $key => $refferal_pay)
                    @php
                        $total += $refferal_pay->refferel_paid;
                    @endphp
                    <tr>
                      <td>{{$key+1}}</td>
                      <td>{{$refferal_pay->patinet->referral->name}}</td>
                      <td>{{$refferal_pay->refferel_paid}}</td>
                      <td>{{$refferal_pay->updated_at->format('d-M-Y')}}</td>
                    </tr>
                    @endforeach
                  </tbody>
                  <tfoot>
    ...
    <script>
        function PrintReport(){
            ...
            if (from_date && to_date) {
              window.open("/app/report/refd_pay_report_print/"+from_date+"/"+to_date,"popup",properties);
            }else{
        ...
        function showReport(){
          ...
          const to_date = $('#to_date').val();
          if (from_date && to_date) {
              window.open("/app/report/refd_pay_report/"+from_date+"/"+to_date,"_self");
            }else{
                alert('Plz selecet the date');
            }
        }
